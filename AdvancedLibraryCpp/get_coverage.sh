...
lcov --directory "${SRC_DIR}" --capture -o "${COV_DIR}/coverage.info" --gcov-tool ./llvm-gcov.sh
...
lcov -r "${COV_DIR}/coverage.info" "*Qt*.framework*"  "*/tests/*" "*Xcode.app*" "*.moc" "*moc_*.cpp" "*/test/*" "*/build*/*" -o "${COV_DIR}/coverage-filtered.info" "*/QtCore/*" "*/QtTest*" "/usr/include/*"
